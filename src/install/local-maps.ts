 ...
 *
 * SSOT semantics: Platform DNA `ensureLocalRepoMaps` (see
 * `platform-dna/docs/PROJECT-MAPS.md`). Vendored here so Processkit init
 * stays independent of install order — never seeds portable
...
/**
 * Route a process step system id to the correct machine-local map.
 * `legacy-*` (or clear legacy prefix) → legacy map; everything else → platform.
 */
 ...
 * Existing files are never overwritten or normalized.
 */
...
/** Status slice for `processkit status` — missing/empty maps for cross-repo. */
