/**
 * Install ledger for Processkit harness destinations.
 *
 * State survives CLI upgrades and lives at:
 *   $PROCESSKIT_STATE_DIR | $XDG_STATE_HOME/processkit |
...
const MANIFEST_REL = '.processkit/install-manifest.json'

export function stateDir(): string {
  if (process.env.PROCESSKIT_STATE_DIR) {
    return path.resolve(process.env.PROCESSKIT_STATE_DIR)
  }
  const base = process.env.XDG_STATE_HOME
    ? path.resolve(process.env.XDG_STATE_HOME)
    : path.join(os.homedir(), '.local', 'state')
  return path.join(base, 'processkit')
}
...
/** Return registered destinations whose Processkit manifest still exists. */
...
function writeLedger(repos: string[]): void {
  try {
    ...
    // Best effort: the ledger accelerates global cleanup but must never break init.
  }
}
...
/** Discover older, ledger-less Processkit installs below a directory. */
  ...
  const walk = (current: string, depth: number): void => {
    ...
    for (const name of entries) {
      ...
      try {
        ...
        // Skip unreadable and disappearing directories.
      }
    }
  }
